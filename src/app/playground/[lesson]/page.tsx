import LessonSections from '@/app/components/lesson';
import { LessonMap, getLessonMDX } from '@/app/components/lesson/data';
...
export default async function Page({ params }: Props) {
  ...
  return (
    <main className='grid h-screen grid-cols-3 bg-sky-50'>
      <header className='col-span-3 grid grid-cols-2 p-2'>
        <div className='flex justify-start gap-2'>
          <span>{'ilayda.com'}</span>
        </div>
        <div className='flex justify-end gap-2'>
          <button className=' bg-blue-500 px-4 py-2 font-bold text-white hover:bg-blue-700 disabled:bg-blue-300'>
            My account
          </button>
